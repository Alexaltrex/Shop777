...
import style from "./setTokenPriceForSell.module.scss";
...
export const SetTokenPriceForSell = observer(() => {
    const {
        cryptoStore: {
        ...
        try {
            if (currentAccountAddress) {
                ...
                const tx = await shopContract
                    .connect(signer)
                    .setTokenPriceForSell(price);
                await tx.wait(); // ждем ее завершения
                await getTokenPriceForSell(); // получить новую цену
            }
        } catch (e: any) {
            errorHandler(e);
        } finally {
    ...
    return (
        <div className={style.setTokenPriceForSell}>
            <Typography className={style.label}>
                Set token price for sell (only owner)
            </Typography>
